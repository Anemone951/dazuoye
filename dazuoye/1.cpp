...
int main() {
    ...
    int current_min = *min_element(files.begin(), files.end());
    int current_max = *max_element(files.begin(), files.end());

    if (current_max - current_min < 2000000) {
        for (int& num_lines : files) {
            if (current_max - current_min >= 2000000) break;
            if (num_lines == current_max) continue;
            num_lines = max_lines;
            current_max = max_lines;
        }
       
        for (int& num_lines : files) {
            if (current_max - current_min >= 2000000) break;
            if (num_lines == current_min) continue;
            num_lines = min_lines;
            current_min = min_lines;
        }
    }
